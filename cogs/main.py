...
class Main(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if result:
            ...
            self.server_event_listener.start()
            ...
            for member in team_info.members:
                if member.is_alive is False:
                    if member.steam_id not in self.dead_list:
                        ...
                        await talk_channel.send(f"{member.name} がしぼうしました。 ({grid})")
                        await self.rust_client.send_team_chat(f"[RUSTBOT] {member.name} がしぼうしました。 ({grid})")
                else:
    ...
    async def server_event_listener(self):
        try:
            talk_channel = self.bot.get_channel(self.talk_channel_id)
            ...
            self.server_event = server_events
        except Exception as e:
            print("Return Error: " + str(e))
